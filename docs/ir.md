...
`softmax_last_axis_f32` only when the stored canonical axis is the final input
axis and returns `plan_operation_unsupported` for every other valid axis. That
backend limitation does not make the graph invalid.
