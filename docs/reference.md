...
1. if any element is NaN, fill the complete slice with canonical quiet NaN
   (`0x7fc00000`);
2. otherwise, if one or more elements are positive infinity, divide unit mass
   equally among those elements and write positive zero elsewhere;
3. otherwise, if every element is negative infinity, fill the complete slice
   with canonical quiet NaN;
4. otherwise, negative infinities contribute exact zero and the finite maximum
   anchors the stable exponential calculation.

NaN takes precedence over positive infinity. These rules are TensorKiln's
documented semantics; they are not an ONNX conformance claim.

The `3 * numel` Softmax work charge is an inclusive deterministic policy bound:
one slice scan, one exponential-and-sum pass, and one normalization pass. A
non-finite slice can return through a cheaper branch but retains the same
charge. Multiplication and aggregate addition are checked before materializing
node payloads.
